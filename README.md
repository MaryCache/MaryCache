...
3.  **Implementation via Context Integration (Human + Multi-AI)**:
    AIの特性に応じた役割分担を徹底する。
    *   **ChatGPT (Intent Context):** 要件やニュアンス（Why）を深く理解しているため、「実装指針」や「IDE用プロンプト」の生成を担当させる。
    *   **IDE AI (Repo Context):** プロジェクト全域のファイル構造（Where）を把握しているため、ChatGPTが生成した指針をもとに、具体的なコーディングを担当させる。
